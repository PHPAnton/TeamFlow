...
<script setup>
    ...
    const onDrop = async (status, evt) => {
        const task = evt.item.__vue__?.element;
    ...
    const addProject = async () => {
        ...
        try {
            ...
            const userId = tokenPayload.sub || tokenPayload.id; // подбери правильный ключ по своей структуре токена
            ...
            // ✅ 4. Обновляем список проектов
            projects.value.push(res.data);
            selectedProjectId.value = res.data.id;
            newProjectName.value = '';
            await loadTasks();
        } catch (err) {
            console.error('Ошибка при создании проекта:', err.response?.data || err.message);
            alert('Ошибка при создании проекта. Убедитесь, что вы авторизованы.');
        }
    };
